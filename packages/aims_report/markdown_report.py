...
from packages.aims_openbim.openbim_validation import OpenBIMValidationReport, validate_openbim
...
def _openbim_section(openbim: OpenBIMValidationReport) -> list[str]:
    lines = ["## OpenBIM / IFC Foundation", ""]
    lines.append(f"**OpenBIM readiness score:** **{openbim.score}/100**")
    lines.append("")
    lines.append("| Metric | Value |")
    lines.append("|---|---:|")
    lines.append(f"| Total entities | {openbim.total_entities} |")
    lines.append(f"| OpenBIM entities | {openbim.openbim_entities} |")
    lines.append(f"| Missing IFC GUID | {openbim.missing_ifc_guid} |")
    lines.append(f"| Missing spatial container | {openbim.missing_spatial_container} |")
    lines.append(f"| Missing PSet properties | {openbim.missing_pset_properties} |")
    lines.append(f"| Unsupported IFC classes | {openbim.unsupported_ifc_classes} |")
    lines.append("")
    lines.extend(_table("OpenBIM IFC Class Summary", openbim.ifc_class_counts))
    return lines
...
def write_markdown_report(
    ...
    openbim = openbim_validation or validate_openbim(document, annotate=False)
    ...
    lines.extend(_openbim_section(openbim))
